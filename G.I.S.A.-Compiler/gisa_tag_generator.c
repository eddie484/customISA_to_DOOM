...
int temp_registration(int bytewidth){
    ...
    temp_table[temp_table_count++] = symbol;

    if (temp_table_count == temp_table_limit) {
        temp_table_limit = temp_table_limit * 2;
...
Node * tag_terminal(Node * ast){
    if ((ast->token.token_number >= 0 && ast->token.token_number < 100) || ast->token.token_number == SEM_SYMBOL || ast->token.token_number == SEM_TYPE) {
...
Node * tag_symbol(Node * ast){
    printf("enter SEM_SYMBOL\n");
    Symbol_info * symbol = symbol_finder_from_symbol_node(ast);
    if (symbol->is_func == 0) {
        printf("심볼이 변수이다.\n");
        ...
        printf("심볼이 함수이다.\n");
        Node * ast_arg_node = ast->brother->son;
        Node * tag_arg_start = NULL;
        Node * tag_arg_last = NULL;
...
Node * tag_nt_function(Node * ast){
    if (ast != NULL && ast->token.token_number == NT_FUNC_DECLR) {
        Node * brother_func = NULL;
        if (ast->brother != NULL && ast->brother->token.token_number == NT_FUNC_DECLR) {
            printf("Processing: brother func declr을 처리합니다.\n");
            brother_func = tag_nt_function(ast->brother);
        }
        if (ast->son->brother->brother->brother == NULL) {
            printf("Processing: tag_nt_function\n");
            printf("Processing: 선언만 있는 함수. TAG tree에 등록되지 않습니다.\n");
        ...
        printf("AST to TAG 과정에서 오류 발생: NT_FUNC_DECLR node를 처리해야 하지만, %d 노드가 입력되었습니다.\n", ast->token.token_number);
        exit(1);
    }
}
...
Node * tag_nt_param_list(Node * ast){
    if (ast->son->son->token.token_number != KW_VOID) {
        ...
        Node * ast_param_brother;
        Node * tag_param_last;

        if (ast_param != NULL) {
            tag_param_last = node_maker(NULL, NULL, ast_param->son->token.token_number, ast_param->son->token.token_value);
            n->son = tag_param_last;
            ast_param_brother = ast_param->brother;
            while (ast_param_brother != NULL) {
                tag_param_last->brother = node_maker(NULL, NULL, ast_param_brother->son->token.token_number, ast_param_brother->son->token.token_value);
                ast_param_brother = ast_param_brother->brother;
                tag_param_last = tag_param_last->brother;
            }
        }
...
Node * tag_nt_instr_interpreting(Node * ast, int temp_in_rA, int temp_in_rB){
    if (ast->token.token_number == KW_RETURN) {
        ...
        if (ast->son->brother->token.token_number == NT_EXP) {
            ...
            return n1;
        } else if (ast->son->brother->token.token_number == OP_LOGIC_NOT) {
            Node * n = line_op_logic_not(ast, temp_in_rA, temp_in_rB);

            return n;
        } else if (ast->son->brother->token.token_number == OP_LOGIC_AND || ast->son->brother->token.token_number == OP_LOGIC_OR) {
            Node * n = line_op_logic_and_or(ast, temp_in_rA, temp_in_rB);

            return n;
        } else if (ast->son->brother->token.token_number >= OP_EQ && ast->son->brother->token.token_number <= OP_GE) {
            Node * n = line_op_comp(ast, temp_in_rA, temp_in_rB);

            return n;
        } else if (ast->son->brother->token.token_number == OP_PRE_INCRE || ast->son->brother->token.token_number == OP_PRE_DECRE) {
            Node * n1 = tag_nt_instr_interpreting(ast->son->brother->brother, temp_in_rA, temp_in_rB);
            Node * n2 = tag_nt_instr_interpreting(ast->son->brother, 0, n1->token.token_value);
            ...
            return n;
        } else if (ast->son->brother->token.token_number == OP_QUESTION) {
        ...
        // Symbol일 경우
            ...
            return tag_symbol(ast->son->brother);
        } else {
...
Node * global_list() {
    ...
    for (int i = 1; i < symbol_id_count; i++) {
        Symbol_info * symbol = symbol_finder_from_symbol_id(i);
        if ((symbol->is_global == 1) && (symbol->is_func == 1 || symbol->init_option == 1 || symbol->init_option == 2)) {
            Node * static_var_node = node_maker(NULL, NULL, TAG_GLOBAL, symbol->name);
    ...
    Node * static_var_list = node_maker(static_var_start->brother, NULL, TAG_GLOBAL_LIST, 0);
    free(static_var_start);
...
Node * tag_generator(Node * parse_input, char * tagtree_name)
{
    ...
    temp_table_limit = 4;
    temp_table_count = 0;
    temp_table = malloc(sizeof(Symbol_info*) * temp_table_limit);
    ...
    tag_global_sym = global_list();
    tag_static_var->brother = tag_top->son;
    tag_global_sym->brother = tag_static_var;
    tag_top->son = tag_global_sym;
